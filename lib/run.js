...
module.exports = async function run(url = './') {
  process.start()
  const ret = getCompsPath(url)
  const [compPath] = ret
  if(!compPath) {
    process.fail(`当前未找到文件夹components`)
    return
  }
  dirs(path.resolve(compPath.path+'components')).then(data => {
    ...
    const content = fs.readFileSync(path.resolve('./lib/temp.js')).toString()
    const result = handlebars.compile(content)({comps:data})
    // console.log(result)
    const compIndexPath = compPath.path+'components/index.js'
    // 写入
